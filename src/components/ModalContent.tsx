...
export const ModalWrapper: React.FC<ModalWrapperProps> = ({
  ...
  return (
    <div className={`${styles.modal} ${isOpen ? styles.modalShow : ''}`} onClick={handleBackgroundClick}>
      <div className={styles.modalContent} style={{ borderRadius: modalBorderRadius, gap: gapStyle }}>
        {children}
      </div>
    </div>
  );
...
export const ModalHeader: React.FC<{ title: string }> = ({ title }) => <h3>{title}</h3>;

export const ModalBody: React.FC<{ message: string }> = ({ message }) => <p>{message}</p>;
